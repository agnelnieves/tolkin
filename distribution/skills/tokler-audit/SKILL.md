...
name: tokler-audit
description: |
  Tokler repo-wide audit skill. Use when the user asks to audit agent-context token
  ...
  "what is my token footprint", "audit my agent files", "tokler project".
metadata:
  version: 0.7.0
---
...
Run a repo-wide agent-context audit using Tokler, interpret the findings, and propose
...
# bunx tokler-cli@latest project . --json
```

If a local release binary is available (e.g. during CI dry-runs), set
`TOKLER_BIN=/path/to/tokler` and the action uses it. The skill always prefers
`npx tokler-cli@latest` in end-user sessions.
...
Key fields from `tokler project . --json`:
...
4. **Check `mcp_configs`** for high `cold_tokens`. If cold tokens are significant,
   recommend running `tokler-slim` to get per-server slim snippets.
...
If the user has usage logs ingested via `tokler stats`, Tier 3 (measured) savings
