---
name: tokler-optimize
description: |
  Tokler full optimization loop skill. Use when the user wants a guided end-to-end
  ...
  footprint", "tokler optimize loop", "full token audit and fix", "run tokler and
...
Run the full Tokler optimization loop: audit, apply safe fixes (with user confirmation),
re-measure, and summarize. Never apply lossy rewrites without explicit opt-in.

## Overview

This skill chains `tokler-audit` and `tokler-slim` into a single guided pass. The loop
is: audit -> propose -> confirm -> apply -> re-measure -> summarize.

## Step 1: Baseline audit

```bash
npx tokler-cli@latest project . --json
...
```bash
npx tokler-cli@latest stats --json
```

Key fields from `tokler stats --json`:
...
If MCP configs were also modified via `tokler mcp`, re-run that command too and
include its delta.
...
Next steps:
  - Run tokler-slim to address MCP cold-load costs (if not already done).
  - Re-run this skill after any significant CLAUDE.md or skill edits.
  - tokler stats --json shows realized savings accumulate over sessions once
...
- **Measured (Tier 3)**: actual spend from ingested agent session logs. Ground truth.
  Only available when `tokler stats` shows ingested usage data.
