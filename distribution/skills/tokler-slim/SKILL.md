...
name: tokler-slim
description: |
  Tokler MCP slim skill. Use when the user wants to reduce MCP server token costs,
  ...
  "reduce MCP tokens", "tokler mcp", "apply slim snippets", "MCP tool-definition cost".
metadata:
  version: 0.7.0
---
...
```bash
npx tokler-cli@latest scan --json
# bunx works identically
```

Key fields from `tokler scan --json`:
...
```bash
npx tokler-cli@latest mcp <path-to-config> --json
```

Key fields from `tokler mcp <config> --json` (shape: `McpAnalysis`):
