...
| I6-3/4/5 + subagent ingestion (wave 2) | completed | delta-vs-base PR gates (action + audit workflow, budget inputs), tools/list ingestion with per-tool counts and smell lints, bench configuration track measures tokenized manifests (schema v2, five vendored fixtures with provenance), measured advisories (model mix, output share, cap runway), subagent-transcript ingestion (+$330.96 surfaced on this machine), adversarial review signed off. 2026-06-11, 0.11.0 |
| I7 wave 3 (Homebrew, hygiene, cache skill, auditor expansion) | completed | Homebrew formula + release job staged (brew install/test/audit clean locally), eight hygiene items incl. JSON-always empty states and the denominator sweep, tolkin-cache skill + hooks templates + PRIVACY.md (drift lint 10 checks), repomix/cavemem measured vs claims + notion-slim forensic not-runnable + BYOK scoring harness ready, adversarial review signed off. 2026-06-11, 0.12.0 |
| I8-1. Local intelligence layer slice 1 + update surface | completed | MLX research verdict executed (GO with scope cut, real M1 Max measurements), sidecar probe and loopback-only client, `tolkin optimize` (deterministic skeleton, dry run, chip-table ETAs, consent class, narration + skill lint behind verification gates), `tolkin update` + consented passive notice, determinism contract tests, PRIVACY.md expansion. 2026-06-11, 0.13.0 |
| I8-2. MCP probe + manifest lint | completed | `tolkin mcp --probe` speaks the real MCP handshake (stdio + streamable HTTP) to the user's own configured servers behind per-server confirmation, exact tokenized manifests cached committably in .tolkin/mcp-manifests with 90-day staleness, basis "measured (probed manifest)" supersedes catalog estimates; `optimize --task mcp` lints the cached index for cross-server paraphrase overlap behind referential + span gates. 2026-06-11, 0.13.0 |
...
| apps/tolkin-cli | yes | yes (cargo) | yes (clippy + fmt) | Rust binary. 12 subcommands plus the bare-`tolkin` Ratatui dashboard (Project/Machine/Spend tabs; `stats --tui`/`--compact`): `count` (with `--verify`), `compare`, `viz`, `redact`, `cost`, `mcp`, `audit`, `drift`, `scan` (per-OS catalog), `project`, `init`, `stats` (three-tier savings, `--global`/`--json`/`--reset`). Local savings ledger + opt-in usage-log ingestion (Claude Code incl. subagent transcripts, Codex; mtime+size parse cache v3 with per-request retention; parent/subagent stream split). `cache` command: measured prompt-cache health (hit rate + advisory, write churn, 5m vs 1h TTL counterfactual, cadence), Claude-Code-sourced. `mcp` ingests tools/list manifests (per-tool tokenized table, five description-smell lints). Measured advisories: model mix, output share, cap runway (monthly_cap_usd). Scan also detects hooks configs (cited PreToolUse/PostToolUse templates offered) and LLM-invoking workflow steps (own bucket). Scripts: portable bump (carries the four skill versions), check-skill-schemas.ts (CI drift lint, 10 checks), cache-recompute.ts (reconciliation harness). 246 tests across 7 suites. |
| apps/tolkin-web | yes | yes (Turbopack) | yes (Biome + oxlint) | Next.js 16 + Tailwind v4 + tsgo. Root `/` is the tolkin.dev marketing landing (dark, Bricolage/Geist/Geist Mono, GSAP intro + typed terminal + stats count-up, Lenis smooth scroll, server-side shiki code blocks, JSON-LD, GA4 in production only). The analyzer moved to `/analyzer` unchanged: shared-state textarea + file dropzone + redaction ledger + 3-provider count panel (with opt-in Verify with Anthropic on the Claude card) + token-chip visualizer + cost calculator (with prices-observed staleness note) + audit panel (Lighthouse-style ranked findings), plus the standalone MCP analyzer. Tokenization and the tolkin-core WASM each run in a dedicated Web Worker; parsers lazy-load. Redaction runs first and feeds every downstream view. The only sanctioned fetch lives in `src/lib/verify/anthropic.ts`. |
...
### 2026-06-11: Wave 2, delta gates + tools/list measurement + advisories + subagent ingestion; adversarially reviewed; 0.11.0

Four parallel worktree agents, one orchestrator integration fix, one adversarial reviewer. The wave's through-line: tolkin stopped estimating where it can now measure, and stopped missing spend it could not see.

#### CI delta gates (A8; the researches' strongest CI idea)

The composite action and tolkin-audit.yml now compare against the PR merge base: a detached git worktree at the base sha, the same binary run against both trees, and a "Change vs base" table (base, head, delta with sign, percent of base, denominator named) for always-loaded tokens, total context, and MCP cold. Opt-in budget gates, default off: `max-always-delta-tokens` and `max-context-delta-pct` action inputs (env knobs TOLKIN_MAX_ALWAYS_DELTA_TOKENS / TOLKIN_MAX_CONTEXT_DELTA_PCT in the workflow), evaluated AFTER the comment posts, exit 3 so logs can tell the delta gate from the severity gate's exit 2; both compose. Baseline unavailable (push events, shallow clones, force-push) renders absolute-only with a quiet note and never fails. A fixture-driven self-test (18 checks: delta math, gate trips, zero-base n/a, no-baseline fallback) gates the extended dry-run workflow. The single top-level env mapping in tolkin-audit.yml is preserved and now carries a warning comment. Dogfood PR with a red delta row follows the merge (entry below).

#### tools/list ingestion and the configuration track upgrade (A9; closes P1-3 the good way)

`tolkin mcp` now ingests a server's real tools/list JSON (explicit `--tools-list <path|->` with `--server` for multi-server configs, plus shape auto-detection): core gains `mcp_tools` (canonical compact serialization of {name, description, input_schema} with sorted schema keys; callers supply per-tool token counts, so tokenization stays platform-native), a per-tool table (name, tokens, share), and five description-smell lints (no-leading-purpose-verb, no-output-shape, over-long at a justified 200-token budget, near-duplicate via the audit MinHash, missing-description), all framed compact-clarity: better descriptions can help success, verbose augmentation increases steps; the lints never recommend lengthening. Exact counts supersede catalog estimates with the basis labeled ("tokenized manifest (o200k_base)" versus "catalog estimate (representative)"); the unknown-server prompt points at the flag (plain output too, after the review). The web MCP panel gained a tools/list paste box (browser tokenizes per tool). The bench configuration track now MEASURES: five live-captured manifests vendored with full provenance (modelcontextprotocol filesystem/memory/everything and github-mcp-server v1.2.0 default plus GITHUB_TOOLSETS=repos,issues slim; licenses vendored; capture.ts rerunnable; captured 2026-06-10 over stdio), results schema v2 (basis, tools, manifest provenance per row; cold = exact tokenized weight, price surcharges no longer folded into token figures), methodology track 2 restored to the manifest claim wave 0 had to soften, /bench flipped accordingly. Honest numbers worth staring at: github v1.2.0's 43 default tools measure 8,175 tokens against the catalog's 40,000 representative figure for the all-toolsets era (both labeled with basis and era); the slim cell is a measured diff of two captured manifests (8,175 minus 4,953 = 3,222); caveman-shrink's configuration comparison flipped from not-comparable to measured at 3.28 percent (descriptions only, input-side). Bench runner spawns now set TOLKIN_NO_LEDGER=1 so measurement infrastructure can never write user state.

#### Measured advisories (A10)

A pure advisories module feeding stats/TUI/report through the shared snapshot: model mix (top model's share of priced spend over 25 percent, or cheapest-tier under 10 percent, rendered as community-heuristic advisories, cited as heuristics, never judgments), output share (output tokens and dollars as a share of the priced bill, with the one framing sentence: this is the most any output-side tool could possibly touch), and cap runway (new optional monthly_cap_usd in config.toml; month-to-date UTC spend plus days-to-cap from trailing 7-day and 30-day burn; silent when unset; requires 3 active days before projecting). 13 hand-computed golden tests pin the threshold boundaries. Real-log smoke: claude-opus-4-7 carries 94.0 percent of priced spend ($1,275.73 of $1,357.67) so the over-concentration advisory fires; output share is 15.3 percent of the bill ($207.83), the measured ceiling for output-side tooling on this machine.

#### Subagent-transcript ingestion (A10b; the wave 1 finding, fixed)

Claude Code nests subagent transcripts at `<session-id>/subagents/agent-*.jsonl`; the walk now reads them. Study findings on real logs: record shape identical to parents (cwd always present), 47 sessions with subagents, 299 files, 13,820 usage records, and ZERO key overlap with parent files (9,078 parent vs 6,332 subagent unique keys; the global last-wins dedup spans both anyway, collision-pinned both directions). Attribution: each subagent file is its own SessionUsage stream (own prompt stream, own prefix; folding into the parent would corrupt churn/TTL math); project attribution by per-record cwd; the parent/subagent distinction lives in UsageData side-tables with parent_session_count()/subagent_stream_count() helpers; parse cache stays v3 (shape unchanged). The orchestrator wired the split into stats: additive parent_sessions and subagent_streams JSON keys and a plain-mode parenthetical, so the headline does not read as a tripled session count. THE DELTA this fix surfaces on this machine: sessions 183 -> 481 (183 working sessions + 298 subagent streams), +6,725 requests, +563.5M cache-read tokens, +20.46M 5m-write tokens (EVERY observed 5m write was subagent traffic main could not see), +3.63M output tokens, and +$330.96 measured cost (1,357.67 -> 1,688.63). The wave 1 cache story updates honestly: with subagent streams counted, the TTL counterfactual narrows from $37.48/$29.29 to $50.57/$50.19 (1h still wins, barely), and the all-writes-at-1h fact becomes a parent-sessions fact.

#### The adversarial review, in writing

A different opus agent attacked both ground-truth movers. Its independent recount matched ALL FIVE manifests token-exact (1,641 / 904 / 1,126 / 8,175 / 4,953) and reproduced the slim diff and the caveman number exactly. Its reconciliation run (after the fixes) matched the recompute script on every numeric field at 482 sessions / 16,099 requests. Sign-off block, verbatim dispositions:

> SIGN-OFF: yes
> 1. Github slim toolset provenance was inaccurate: the README claimed the default context toolset stays registered when GITHUB_TOOLSETS is set; upstream v1.2.0 gates the other default toolsets off (verified in pkg/github/tools.go), and the captured fixture correctly has no context tools. The measured tokens and slim diff are correct; only the prose was wrong. FIXED (9abde54), bench artifacts regenerated.
> 2. Plain-text mcp report dropped the unknown-server note (JSON-only). FIXED (4b0a24b) with a regression test pinning the rendered text.
> 3. Cache-analysis inter-session gap semantics drifted silently under subagent streams: the wave 1 rustdoc claimed the gap measured cache-survival opportunities, which a parent-to-subagent pair is not. Numbers reconcile; the documented interpretation was corrected (15e668e) plus three new regression tests (parent-wins collision side-tables, orphan subagents dirs, zero-usage subagent files).

Attacks survived without change: exactness end to end, canonical serialization equals docs, wire-bytes caveat present, basis labeling coherent across surfaces, adversarial manifests (boundary tokens, unicode, duplicate names, string schemas, deep nesting; no panics), schema v2 rendering, determinism twice, TOLKIN_NO_LEDGER on every runner spawn (grep plus an observed-empty data dir), --provider back-compat, headline split consistency. One noted non-regression gap: duplicate tool names in one manifest draw no lint (queued as polish).

#### Verification (combined tree, orchestrator-run)

| Check | Result |
|---|---|
| cargo test (cli) | 222 pass across all suites (+2 ignored real-log probes) |
| cargo test (core) | 118 pass |
| clippy + fmt + cargo-deny, both workspaces | clean |
| wasm-pack release build | pass |
| web gates (lint, lint:fast, typecheck, build) | all pass |
| skill schema drift lint | 7/7 green (additive keys only) |
| bench determinism | holds (rerun on the final tree; artifacts restored, timestamp-only drift) |
| dash scan / egress scan / bun pm untrusted | 0 / 0 / clean |
| Browser pass | SSR curl: home renders the tools/list paste section; /bench renders the manifest-measured track copy, provenance list, and the 8,175 figure |
...
### 2026-06-11: Wave 3, Homebrew + hygiene + the fourth skill + the auditor expansion; 0.12.0

Four agents (one serialized behind the hygiene batch to share the scan files), one adversarial reviewer over the published-numbers work, two orchestrator hardening fixes.

#### Homebrew staged end to end (A11)

`distribution/homebrew/Formula/tolkin.rb` (binary formula, four platform blocks, placeholder checksums, commented livecheck) plus the tap README, a Homebrew section and support-matrix row in the public README, and a `homebrew-release` job appended to tolkin-publish.yml (soft until the owner actions land): tars the four non-Windows binaries from the run's artifacts, writes SHA256SUMS, creates the GitHub Release on the public companion repo, and updates the tap formula, all with gh and coreutils, no third-party actions, authenticated by the future HOMEBREW_TAP_TOKEN fine-grained PAT. Validated LOCALLY before any owner action: a scratch formula with file:// URLs against a locally built tarball installed via `brew install --formula`, passed `brew test` (version assertion) and `brew audit --strict` with ZERO findings, then uninstalled clean. Orchestrator hardening: the tap-update version substitution was a literal frozen at 0.11.0 (correct for release one, silently stale at release N); it is now a generic semver match, dry-run proven against a 0.12.0 substitution.

#### The hygiene batch (A12, eight items)

MCP_CATALOG_OBSERVED (2026-06) prints next to catalog estimates, the PRICES_OBSERVED pattern applied to the catalog. Scan discovers `.windsurf/rules/` and `.clinerules` (per-OS tests; promptfoo/litellm deliberately excluded with the reasoning in a comment) and gains a `.github/workflows` detector that counts prompt-bearing fields of LLM-invoking steps in its own `workflows_llm` bucket, never folded into always-loaded (per-CI-run, not per-session; the note says so). `stats --json` and `cache --json` now ALWAYS emit valid JSON (zero/null schema plus a hints array) on fresh machines, empty ledgers, and consent-off states; the skills teach agents to run exactly these commands. The Tool-Search note carries the Arcade.dev retrieval caveat (regex variant 56 percent, 14 of 25, 4,027-tool corpus; BM25 64 percent; a missed tool cannot be called; URL cited) while stating the token figure is unaffected. The duplicate-tool-name manifest smell landed (the wave 2 reviewer's noted gap). The TTL counterfactual carries the per-model 1h-availability footnote (Bedrock URL). And the denominator sweep audited 21 rendered percentages across CLI, TUI, report, and web: 16 already named their denominator, 5 fixed (cache hit rates now say "of input-side tokens" on three surfaces; two bare "Share" column headers now name their base).

#### The fourth skill, hooks, and the privacy doc (A13)

`distribution/skills/tolkin-cache/SKILL.md` wraps `tolkin cache --json` with the tier-correct counterfactual reading (ground-truth gaps, advisory-estimate simulations, the Claude Code scope line), applies prefix-stability fixes only with confirmation, and re-measures; its schema blocks are live-generated, annotated, and covered by the drift lint (now 10 checks; the plant-a-fake-key proof ran fail-then-green again), and the bump script carries the fourth version field. Scan detects hook configurations (presence and configured events only) and, when none exist, offers two copy-pasteable cited templates (PreToolUse guard, PostToolUse truncation; canonical hooks doc URL verified through its redirect), never auto-installed. `distribution/PRIVACY.md` states the posture with every claim mapped to the implementing file, including the honest disclosure that ledger records carry the project's absolute path; linked from the public README.

#### The auditor expansion (A14 plus adversarial review)

The claimed-versus-measured ledger grew, every number on declared fixtures with vendored licenses and provenance: repomix --compress (claimed about 70 percent, basis undisclosed) measures 45.47 percent on a three-file declared corpus, denominator-sensitivity stated; cavemem (the research's unverified figure) measures 11.76 percent on the lossy fixtures at the tool's default intensity; notion-slim ships NOT-RUNNABLE with forensic precision (the npm tarball carries only Windows binaries despite README claims, index.js references absent darwin/linux paths, zero GitHub releases, closed-source transformation), alongside the FULL Notion manifest measured at 15,698 tokens (22 tools, superseding the catalog's 26,000 estimate) and the implied 7,535-token target if the 52 percent claim holds, so any third party with a Windows box can finish the measurement. The BYOK extraction-QA scoring harness is fully wired and off by default: one command (`ANTHROPIC_API_KEY=<key> bun apps/tolkin-cli/benchmarks/run.ts --score-quality`), a deterministic grader (no LLM-as-judge), scored=false everywhere until the owner runs it. caveman-shrink's configuration row honestly diluted from 3.28 to 1.89 percent when the Notion manifest joined the corpus (math verified).

The adversarial reviewer reproduced every published number byte-exactly, verified all six notion-slim forensic claims against the actual tarball and registry, confirmed determinism twice, the dependency posture (repomix 67 + cavemem 47 dev-only packages, postinstalls bun-blocked, the harness runs without the native binding), and the off-by-default scoring. One P0 break, fixed: the cavemem row attributed the about-46-percent figure to the upstream README, whose only published figure is about 75 percent for prose; the 46 percent is an external research-note number and the row now says exactly that, measured value unchanged. SIGN-OFF: yes.

#### Verification (combined tree, orchestrator-run)

| Check | Result |
|---|---|
| cargo test (cli) | 246 pass across 7 suites (+2 ignored real-log probes) |
| cargo test (core) | 121 pass |
| clippy + fmt + cargo-deny, both workspaces | clean |
| wasm-pack release build | pass |
| web gates (lint, lint:fast, typecheck, build) | all pass |
| skill schema drift lint | 10/10 green (four skills covered) |
| bench determinism | holds on the final tree |
| dash scan / egress scan / bun pm untrusted | 0 / 0 / clean |
| brew validation | install + test + audit --strict clean on this machine (formula at file:// URLs) |

Bumped 0.11.0 -> 0.12.0 (minor: cache skill, hooks scan, workflows detector, bench expansion; Homebrew job staged soft).

#### The owner-action batch (the engagement's remaining stop points, consolidated)

1. Create the public companion repo `agnelnieves/tolkin` from distribution/ (the long-standing I5 action; now ALSO the binary release host for Homebrew). Replace the three `<public-repo>` placeholders; tag the action v1.
2. Create `agnelnieves/homebrew-tolkin` (empty is fine; the publish job seeds Formula/tolkin.rb and README from distribution/homebrew/ on the first release, or copy them manually).
3. Create a fine-grained PAT with contents read/write on exactly those two repos; store as the `HOMEBREW_TAP_TOKEN` secret on this repo. Then remove `continue-on-error` from the homebrew-release job and enable the formula's livecheck block.
4. ANTHROPIC_API_KEY repo secret (unblocks the weekly drift watchdog, carried over since I2) and, locally, the one-command scored bench run above so /bench can publish scored=true once.
5. One human pass over the TUI in a real terminal (carried over from I3; the Spend tab gained the cache advisory row in 0.10.0 and the advisory lines in 0.11.0, so one pass covers all of it).

### 2026-06-11: 0.12.0 shipped; the I6/I7 engagement's autonomous scope is complete

The wave 3 merge published all six packages at 0.12.0 (CI and the npm publish both green; the retry-hardened verify did its job quietly). The new homebrew-release job soft-failed exactly as designed for the secret-less state, but its dry contact with reality caught one real defect before the owner ever flips it strict: it downloaded artifacts by bare target name while the build job uploads them as tolkin-<target>; fixed on the spot. Engagement scoreboard across the four waves: 0.9.1 (every review P0/P1 fixed and proven), 0.10.0 (the cache layer, adversarially signed off, reconciled twice), 0.11.0 (delta gates proven on a live PR, manifest-measured benchmarks, measured advisories, the subagent ingestion fix worth +$330.96 of visible spend), 0.12.0 (Homebrew staged and locally brew-validated, the hygiene batch, the fourth skill plus PRIVACY.md, and the claimed-versus-measured expansion where repomix's about-70-percent claim measured 45.47 and the research-cited cavemem figure measured 11.76, all reproduced byte-exactly by an independent reviewer). Everything that remains is the owner-action batch above.

### 2026-06-11: Public launch, owner-delegated: both repos created, first release cut, brew install live

The owner delegated the repo creation to the session ("create the repos yourself with the github cli"). Executed: (1) the three README placeholders replaced with the real slug and the fourth skill added to the skills list; (2) github.com/agnelnieves/tolkin created public and seeded with a FRESH single-commit snapshot of distribution/ (no private history; commit c803772), tagged v1 and v1.0.0 for the action; (3) github.com/agnelnieves/homebrew-tolkin created public; (4) the first binary release cut by hand from the 0.12.0 publish run's own CI artifacts (the four non-Windows binaries tarred per the homebrew job's recipe, SHA256SUMS, release v0.12.0 on the companion repo), the darwin-arm64 binary smoke-printing tolkin 0.12.0 before upload; (5) the tap seeded with the formula at the real version and the four real checksums, substituted in URL order and ruby-syntax-checked. The end-to-end proof, run on this machine against the LIVE tap and release: `brew install agnelnieves/tolkin/tolkin` installed, `tolkin --version` printed tolkin 0.12.0, `brew test tolkin` passed, then uninstall and untap left the machine clean. Homebrew distribution is live for real users as of this entry.

Owner actions remaining after the delegation: the HOMEBREW_TAP_TOKEN fine-grained PAT (web UI only, GitHub ships no API for PAT creation; contents read/write on exactly the two public repos; then flip the homebrew-release job strict and enable livecheck so FUTURE releases flow from CI), the ANTHROPIC_API_KEY secret (owner said they will handle it) plus the one-command scored bench run, and the human TUI pass.

### 2026-06-11: Secrets wired and proven; one blocker left and it is billing, not plumbing

The owner configured both repo secrets (HOMEBREW_TAP_TOKEN, ANTHROPIC_API_KEY; no local keys, GitHub only). Shipped in response: the homebrew-release job went STRICT (its soft-mode comment replaced with the dated rationale), livecheck enabled in both the staged formula template and the live tap (tap commit bfbfc6f), and tolkin-bench.yml gained a `score` dispatch input that keys the extraction-QA harness from the repo secret, so scored=true runs never need a local key. Both API-consuming workflows were then dispatched as proof: the drift watchdog (its first keyed run ever, pending since I2) and the scored bench. Both authenticated, reached the Anthropic API, and failed identically on HTTP 400 "credit balance is too low" (a billing state, not an auth failure; request ids in the run logs). The plumbing is end-to-end proven; the account behind the key needs credits. Retry, once credits exist: `gh workflow run tolkin-drift.yml` and `gh workflow run tolkin-bench.yml -f score=true`, then commit the scored artifacts from the bench run. Remaining owner items: credits on the Anthropic account, and the human TUI pass. Minor hardening noted, not done: the drift script prints the API error clearly but then also raises a JSON decode traceback on the failed response body.

### 2026-06-11: 0.13.0, the local intelligence layer ships its first slice

The MLX research commissioned on 2026-06-10 was executed and answered first: MLX-RESEARCH-FINDINGS.md returns GO with scope cut, on real measurements taken on this machine (M1 Max, Qwen3.5-4B-4bit via mlx-lm 0.31.3: prefill plateaus near 250 tok/s, so the 22.7k-token full bundle costs about 95 seconds here and an estimated 4 to 6 minutes on a base M2; JSON came back valid in 5 of 6 runs with the single failure a detectable max-tokens truncation). The verdict shaped the build: task-scoped advisory work, never monolithic repo prompts, and the model proposes while Rust verifies.

Slice 1 shipped the same day across two agent waves. Wave 1: src/sidecar.rs (probe 8080/11434/1234, backend identification, loopback enforced in code with TOLKIN_NO_SIDECAR and TOLKIN_SIDECAR_ALLOW_REMOTE escapes, OpenAI-compatible chat client with truncation detection), the consent and config fields (consent_local_model asked lazily, consent_update_check asked at init and defaulting to no), `tolkin update` (one GET to the npm registry, install-channel detection so Homebrew users see brew upgrade and npm users see npm update), and tests/determinism.rs, eleven tests proving every deterministic command stays byte-identical under hostile sidecar env. Wave 2: `tolkin optimize` (deterministic skeleton always prints; --dry-run with --show-prompts makes zero chat calls; ETAs computed from pre-call token counts against a chip rate table seeded with the measured numbers; narration gated by number faithfulness, every digit in model output must exist in the input JSON or a small whitelist; skill lint gated by span grounding, strict enums, and one truncation retry; all file content passes secret redaction before any prompt, proven by a test that plants an API key), plus the consent-gated once-a-day passive update notice on stderr. Model output renders only as "model advisory (local)" with an advisory-only footer, never a tier.

Proof on this machine against a fake OpenAI-compatible loopback server: the consented path produced a verified narration and a span-grounded lint finding on a fixture skill; the dry run hit the server with exactly one GET /v1/models and zero POSTs; the unconsented non-interactive path reported not_asked and called nothing. Gate at merge: 348 tests green, clippy -D warnings clean, rustfmt clean, skill schema lint 10 of 10 at 0.13.0. One engagement note: the first wave 2 agent run died emitting an oversized final message with zero code loss, and the relaunch with checkpoint commits landed everything; the lesson entry is in LESSONS.md.

Next, in order: the base-M2 spike from the findings before any fleet messaging (kill criteria stand: narration faithfulness under 95 percent, P50 over 120 seconds on a 16GB M2, lint usefulness under half), the distribution README and action version sync (still reading 0.9.0), the tolkin-optimize skill closed loop (prefer the local layer over burning harness tokens when a sidecar is present), and embeddings-first paraphrase dedup as slice 2. The 0.13.0 publish itself rides the existing release flow when the owner cuts it.

### 2026-06-11: the probe makes custom MCP servers exact; the model lint covers what MinHash cannot

The owner's question was direct: unrecognized servers in the scan, custom and internal ones at work, could the local model help. The answer inverted the premise: recognition by model guesswork is the wrong tool when measurement is available, so the deterministic path shipped first. `tolkin mcp --probe <server>` (and --probe-all) speaks the real MCP handshake, initialize then tools/list with pagination, over both stdio (executing the user's own configured command) and streamable HTTP (contacting the configured URL with the configured headers). Per-server confirmation prints exactly what will run, env variable names shown and values never; CI refuses the probe outright and points at committed manifests. The captured manifest passes secret redaction and lands in .tolkin/mcp-manifests/<slug>.json, date-stamped and committable, so one teammate probes once and everyone's `tolkin mcp` reports basis "measured (probed manifest, captured <date>)" with exact per-tool token counts, superseding catalog estimates, with a staleness warning past 90 days. Proven on this machine against a fake HTTP MCP server: probe captured a 3-tool manifest (a planted fake API key arrived redacted in the cache), the rerun without --probe hit the cache and printed the measured basis.

The model layer then took only the semantic remainder. `optimize --task mcp` builds a compact index of all cached manifests (capped near 5k tokens with whole-server omission accounting) and asks the local model for exactly what deterministic heuristics cannot judge: cross-server paraphrase overlap, whole-server redundancy, vague tool names. The rubric itself tells the model not to re-report the deterministic description smells. Gates: span grounding against the index text plus referential grounding (every server and tool named must exist in the index; overlap findings without a related tool are dropped). The live proof was adversarial by design: the fake model returned one real cross-server overlap and one fabricated finding naming a nonexistent server; the gate dropped the fabrication and kept the real finding, dropped_findings 1, exit 0.

Gate at close: 382 tests green, clippy -D warnings clean, rustfmt clean, determinism contract unmodified. PRIVACY.md gained the user-directed probe class (requests go only where the user's own config already points; nothing tolkin chose). Next: the owner probes the real internal servers on the work machine and commits the manifests; legacy SSE-only transport stays out of scope until a real server needs it; slice 2 (embeddings dedup, contradiction audit, the skill closed loop) unchanged.

### 2026-06-11: 0.14.0, optimize meets the user where the model is not installed yet

The no-sidecar path of `tolkin optimize` stopped being a dead end. It now detects the machine's RAM and chip, recommends the blessed model for that tier straight from the findings table (8GB gets the 0.8B with an honest quality warning, 16GB the 4B floor model, 32GB the 9B, 48GB and up the 35B-A3B MoE), prints the download size and a per-machine narration estimate from the chip rate table, and asks one question: "Would you like to see how to install and set it up?" A yes prints the tailored guide, opening with "Nothing leaves your machine", numbering the uv plus mlx-lm steps with the exact model id, naming LM Studio as the app path, and closing with removal instructions. Non-interactive and --json runs never prompt; --json gains a stable suggested_model object instead. Proven interactively on this machine through a pty: the 64GB box was recommended the MoE at 20.4 GB with a 35s narration estimate, and the y path printed the full guide. Gate: 403 tests, clippy clean against the newer CI toolchain, schema lint 10 of 10 at 0.14.0. Released the same hour through the CI train.

### 2026-06-11: tolkin.dev landing page; the analyzer moves to /analyzer

The web app's root stopped being the analyzer and became the marketing landing for tolkin.dev. The analyzer kept its exact behavior at `/analyzer` (the route file re-renders the same client components; no analyzer code changed). `/bench` untouched. Eleven landing sections under `src/components/landing/`: sticky nav with a brew-install copy chip and a mobile disclosure, hero (Bricolage display headline, install tabs for brew/npm/bunx, a typed terminal running the real `tolkin project --json | jq .totals` shape), a stats band counting up the measured numbers (0.97522 cache hit rate, $8.19 TTL counterfactual, 169 reconciled sessions, 22-server catalog), the eight-command features grid, the three honesty tiers with the model-advisory fourth class, the local intelligence section (optimize's no-model recommendation flow, the four-line mlx-lm setup, the RAM tier table, the Rust verification gates), MCP probe exactness with the measured basis string, claimed-vs-measured rows (cavemem 11.72 percent, repomix 45.47 percent) linking /bench, the privacy section (three egress classes, four kill switches, PRIVACY.md link), the distribution band, and the footer.

Plumbing: fonts via next/font (Bricolage Grotesque display, Geist body, Geist Mono code), metadataBase tolkin.dev with title template and canonicals, robots.ts + sitemap.ts (/, /analyzer, /bench), a typographic opengraph-image, JSON-LD SoftwareApplication at 0.14.0, GA4 (G-8QPMMZHYKZ) rendered only when NODE_ENV is production. Motion respects prefers-reduced-motion everywhere (gsap.matchMedia, a Lenis bail-out, instant stats and terminal); the hero intro plays once per session; transform and opacity only. Code blocks are server-side shiki (poimandres, transparent background) so no highlighter ships to the client. Gates: build, Biome, oxlint, tsgo all clean; / and /analyzer curl-verified 200 with expected strings; the dash scan stays at zero. One config fix riding along: tsconfig dropped `baseUrl` (removed in tsgo) in favor of bare `paths`.

Deployment addendum, same day: the site exports statically (every route is prerendered), so production deploys ship as plain files via the Build Output API (vercel deploy --prebuilt), no server build and no monorepo staging on the deploy host. Live at https://tolkin.dev with /, /analyzer/, /bench/, sitemap, robots, and the OG image all 200; GA4 tag present in production HTML only. The Vercel project is "tolkin" under the personal scope; the domain was registered on Vercel yesterday so DNS was instant.

Motion and instrument pass, same day: the landing gained masked line reveals on every display heading (SplitText lines behind overflow masks, fonts-ready gated, screen-reader text preserved), a lazily mounted three.js particle field behind the hero (14k points, DPR capped at 1.75, render loop removed whenever the hero leaves the viewport or the tab hides, absent under reduced motion; deferred chunk, not in first-load JS), nav duplicated-text hover slides, a 1:1 scroll progress hairline, and proper Lenis-to-ScrollTrigger wiring through the gsap ticker. The analyzer was redesigned as an instrument on the site design system (two-column working surface, one-click sample chips, plain-language panel explainers, every consent and disclosure string verbatim, zero behavior change). The bench page became the evidence page: "Claims, measured." with claimed-versus-measured bar pairs from the real results JSON, honesty badges, provenance line. First-load JS cost of the motion pass: +21.7 kB gzip; three.js stays out of the critical path. All three pages live on tolkin.dev.

### 2026-06-11: 0.15.0, the dashboard becomes a product surface instead of a status page

The TUI revamp commissioned with "make it OpenCode-good" shipped end to end on `feat/tolkin-tui` (worktree off feat/tokenly, pending merge). The work started from a researched design contract (TUI-DESIGN.md: a brief on OpenCode's actual TUI internals, surface layering over borders, Radix-style 12-step scales, an animation inventory with exact timings, and a hard-contracts section that protected every agent-facing seam), then ran as three implementation waves, a three-lens adversarial review, a fix wave, and an orchestrator polish pass. What exists now: an Elm-shaped app (input thread plus worker threads over mpsc, derived data recomputed only on data messages, 33 ms cadence only while something animates, 1 s idle), a real theme system (tolkin-dark, tolkin-light, terminal, mono; truecolor with 256-color degradation; NO_COLOR forces mono; TOLKIN_THEME and a `t` cycle that persists ui_theme through the existing config path), a clock-injected animation engine with TOLKIN_REDUCED_MOTION snapping everything to truth, and a component library (cards, select lists with rails and scrollbars, bar rows, gauge, modals with a scrim, palette, help, toasts, scanner spinner).

The dashboard itself grew a fourth tab and lost its passivity. Overview (new default) answers the two-second question with count-up stat cards, the 30-day spark, top advisories, and machine facts. Every list selects and drills: file detail runs a worker-backed audit on demand (`a`), project detail charts snapshots over time, advisory detail prints the full reasoning, the Spend day strip has an h/l cursor with exact per-day numbers. ctrl+k opens a fuzzy palette that doubles as keybinding discovery, `?` teaches humans the keys and agents the CLI equivalents (stats --json, stats --compact, exit codes, the env contract), toasts confirm refresh, scan, OSC52 copy, and report generation, and the mouse works (wheel, click-select, click-outside-closes). The honesty line and tier labels render verbatim on every numeric surface; the compact frame is now byte-deterministic and test-proven so agents can diff it.

Review caught what reviews exist to catch: a Machine list that styled all rows to blit twenty, a footer that dropped the prices date at 80 columns, a Project tab that clipped the docs bar at the 80x24 floor, an as-of column that overflowed on old timestamps, a raw-mode leak on partial setup failure, and worker panics that could restore the terminal under a live loop; all six plus four smaller findings and three weak-test rewrites landed as one fix wave. Proof at close: 540 tests (137 over the 0.14.0 baseline), clippy -D warnings on the newer CI toolchain, rustfmt, cargo-deny all clean, and a real-pty expect drive (tab walk, overlays, palette typing, theme cycle, refresh, scan, quit) that exited 0 with the alternate screen entered and left exactly once, the cursor restored, zero panics, and ui_theme written by the live binary. Bumped 0.14.0 to 0.15.0 across all nine carriers. Remaining: the owner's eyeball pass over the new dashboard (the standing item this work was for), then the merge into the landing branch and the release train.
